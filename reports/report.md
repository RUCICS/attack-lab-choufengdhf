...
```asm
# ========== 函数开头：保存 Canary ==========
136c:  mov    %fs:0x28,%rax        # 从 TLS 读取 Canary 值
1375:  mov    %rax,-0x8(%rbp)      # 保存到栈上 (rbp - 8)

# ========== 函数逻辑 ==========
# ...  中间代码 ... 

# ========== 函数结尾：检查 Canary ==========
140a:  mov    -0x8(%rbp),%rax      # 读取栈上保存的 Canary
140e:  sub    %fs:0x28,%rax        # 与原始值比较
1417:  je     141e                 # 相等则正常返回
1419:  call   __stack_chk_fail     # 不相等则调用失败处理函数
141e:  leave
141f:  ret
```
- **解决方案**：
